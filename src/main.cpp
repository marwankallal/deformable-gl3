...
// Keep around previous shell index
static vector<vector<Cvec3>> g_prevShellCoords;
...
static void initBunnyMeshes() {
	...
	for (int i = 0; i < g_bunnyMesh.getNumVertices(); i++) {
		g_bunnyMesh.getVertex(i).setNormal(Cvec3());
	}

	for (int i = 0; i < g_bunnyMesh.getNumFaces(); i++) {
		Mesh::Face f = g_bunnyMesh.getFace(i);
		const Cvec3 f_norm = f.getNormal();

		for (int j = 0; j < f.getNumVertices(); j++) {
			Mesh::Vertex currentVertex = f.getVertex(j);
			currentVertex.setNormal(currentVertex.getNormal() + f_norm);
		}
	}
	for (int i = 0; i < g_bunnyMesh.getNumVertices(); i++) {
		Mesh::Vertex v = g_bunnyMesh.getVertex(i);
		Cvec3 v_norm = v.getNormal();
		v.setNormal(v_norm.normalize());
	}
	...
	for (int i = 0; i < g_bunnyMesh.getNumFaces(); i++) {
		...
		for (int j = 1; j < f.getNumVertices() - 1; j++) {
			...
			norms[1] = f.getNormal();
			norms[2] = f.getNormal();
			verts.push_back(VertexPN(f.getVertex(0).getPosition(), norms[0]));
			verts.push_back(VertexPN(f.getVertex(j).getPosition(), norms[1]));
			verts.push_back(VertexPN(f.getVertex(j + 1).getPosition(), norms[2]));
		}
	}
...
static void drawArcBall(Uniforms& curSS) {
	...
	curSS.put("uColor", Cvec3(0.27, 0.82, 0.35)); // set color
	g_arcballMat->draw(*g_sphere, curSS);
}

static VertexPNX findShellVertex(Mesh::Face f, int v_idx, int layer, RigTForm inv_bunny, Cvec2 tex) {
	
	Mesh::Vertex v = f.getVertex(v_idx);
	 Cvec3 p = v.getPosition();
	 Cvec3 n = v.getNormal() * (g_furHeight / g_numShells);
	 Cvec3 t = inv_bunny * g_tipPos[v.getIndex()];
	 Cvec3 d = ((t - p - (n * g_numShells)) / ((g_numShells * g_numShells - g_numShells) / 2));

	Cvec3 prev_pos;
	Cvec3 norm;
	if (layer == 0) {
		prev_pos = v.getPosition();
		norm = v.getNormal();
	}
	else {
		prev_pos = g_prevShellCoords[v.getIndex()][layer - 1];
		norm = (prev_pos + n + d * layer) - g_prevShellCoords[v.getIndex()][layer - 1];
	}

	g_prevShellCoords[v.getIndex()][layer] = prev_pos + n + d * layer;

	return VertexPNX(prev_pos + n + d * layer, norm, tex); 
}

static void findHairVelocities(Mesh::Vertex vert, RigTForm bunny) {
	int v_idx = vert.getIndex();
	double T = g_timeStep;
	Cvec3 hair_root = bunny * vert.getPosition();
	Cvec3 tip_rest = bunny * (vert.getPosition() + vert.getNormal() * g_furHeight);
	Cvec3 tip = g_tipPos[v_idx];
	Cvec3 vel = g_tipVelocity[v_idx];
	Cvec3 f = g_gravity + (tip_rest - tip) * g_stiffness;
	
	tip += vel * T;
	g_tipPos[v_idx] = hair_root + ((tip - hair_root) / norm(tip - hair_root)) * g_furHeight;
	g_tipVelocity[v_idx] = ((vel + f * T) * g_damping);
}
...
static void updateShellGeometry() {
	...
	RigTForm inv_bunny = inv(getPathAccumRbt(g_world, g_bunnyNode));

	g_prevShellCoords.resize(g_bunnyMesh.getNumVertices());
	for (int i = 0; i < g_bunnyMesh.getNumVertices(); i++) {
		g_prevShellCoords[i].resize(g_numShells);
	}
	...
	for (int i = 0; i < g_numShells; ++i) {
		...
		for (int j = 0; j < g_bunnyMesh.getNumFaces(); j++) {
			...
			verts.push_back(VertexPNX(findShellVertex(f, 0, i, inv_bunny, Cvec2(0, 0))));
			verts.push_back(VertexPNX(findShellVertex(f, 1, i, inv_bunny, Cvec2(g_hairyness, 0))));
			verts.push_back(VertexPNX(findShellVertex(f, 2, i, inv_bunny, Cvec2(0, g_hairyness))));
		}
...
static void drawStuff(bool picking) {
	...
	const Cvec3 eyeLight1 = Cvec3(invEyeRbt * Cvec4(g_light1Node->getRbt().getTranslation(), 1));
	const Cvec3 eyeLight2 = Cvec3(invEyeRbt * Cvec4(g_light2Node->getRbt().getTranslation(), 1));
...
static void hairsSimulationCallback(int dontCare) {
	...
	for (int i = 0; i < g_numStepsPerFrame; i++) {
		for (int j = 0; j < g_bunnyMesh.getNumVertices(); j++) {
			findHairVelocities(g_bunnyMesh.getVertex(j), bunny);
		}
	}
...
static void initSimulation() {
	...
	for (int i = 0; i < g_bunnyMesh.getNumVertices(); i++) {
		g_tipPos[i] = g_bunnyMesh.getVertex(i).getPosition() + g_bunnyMesh.getVertex(i).getNormal() * g_furHeight;
	}
...
static void initScene() {
	...
	g_light1Node->addChild(shared_ptr<SgGeometryShapeNode>(
	...
	g_light2Node->addChild(shared_ptr<SgGeometryShapeNode>(
		new MyShapeNode(g_sphere,
			g_lightMat, // USE MATERIAL as opposed to color
			Cvec3(0, 0, 0),
			Cvec3(0, 0, 0),
